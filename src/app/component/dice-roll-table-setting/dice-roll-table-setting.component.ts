...
コマンドの例）
　ShipType=3
　前述のダイスボット表の例「遭遇艦種」を数字3指定で参照します、結果「重巡洋艦」が表示されるでしょう。1未満や6を超える数字を指定した場合「（結果なし）」となるでしょう。

　ShipType+2
　前述のダイスボット表の例「遭遇艦種」を1d6の結果に+2した数字で参照します。「遭遇艦種」には7以降の記載がありませんので、その場合「（結果なし）」となるでしょう、それを避けたい場合は前述のワイルドカード * を使用します。`;
  }
}
