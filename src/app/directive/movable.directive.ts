...
export class MovableDirective implements AfterViewInit, OnChanges, OnDestroy {
  ...
  set posX(posX: number) { this._posX = this.mathFloor? Math.floor(posX):posX; this.setUpdateBatching(); }
  get posY(): number { return this._posY; }
  set posY(posY: number) { this._posY = this.mathFloor? Math.floor(posY):posY; this.setUpdateBatching(); }
  get posZ(): number { return this._posZ; }
  set posZ(posZ: number) { this._posZ = this.mathFloor? Math.floor(posZ*8)/8:posZ; this.setUpdateBatching(); }
  ...
  onInputStart(e: MouseEvent | TouchEvent) {
    ...
    if ((this.isDisable && !this.isScratcOwner )|| (e instanceof MouseEvent && (e.button !== 0 || e.ctrlKey || e.shiftKey))) {
      this.cancel();
      return;
    }
